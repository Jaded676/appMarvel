angular.module('comicsMarvel.controllers', ['ui.bootstrap'])
  ...
  .controller('AppCtrl', ['$rootScope','$scope', 'ArticlesService','$state',
    function($rootScope, $scope, ArticlesService,$state) {
      ...
      $scope.searchByYear = function (startYear) {
        var promise = ArticlesService.getComics(false,startYear);
        promise.then(
          function(comics){
      ...
      $scope.searchByTitle = function (titleComic) {
        var promise = ArticlesService.getComics(titleComic,false);
        promise.then(
          function(comics){
            $rootScope.articles = comics.data.data.results;
            console.log($rootScope.articles);
